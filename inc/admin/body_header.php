...
<table border="0" width="1000px" height="100%" align="center" id="maintable" class="maintable">
	<tr>
		<td align="center" colspan="2" height="80px" valign="top" style="border-bottom: 1px dotted #666666;">
			...
			<div class="dbver">Core Version: <?php echo $Core->version; ?> || Expected Database Version: 
			<?php 
			$db_act_ver = $DB->selectCell("SELECT dbver FROM iceweb_version");
			if($db_act_ver < $Core->exp_dbversion) { echo "<font color='red'>".$Core->exp_dbversion." (<a href=\"index.php?p=admin&sub=updates\" /><small>Needs Updated</small></a>)</font>";
			}elseif($db_act_ver > $Core->exp_dbversion) { echo "<font color='red'>".$Core->exp_dbversion." (<a href=\"index.php?p=admin&sub=updates\" /><small>Database outdates the core!</small></a>)</font>";
			}else{ echo $Core->exp_dbversion; } 
			?>
