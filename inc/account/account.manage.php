...
else
{
	...
	elseif($_GET['action']=='changeemail')
	{
        ...
        if($auth->isvalidemail($newemail))
		{
            if($auth->isavailableemail($newemail))
			{
				...
				{
                    output_message('success','<b>'.$lang['change_mail'].'</b><meta http-equiv=refresh content="2;url=index.php?n=account&sub=manage">');
                }
            }
			else
			{
                output_message('validation','<b>'.$lang['reg_checkemailex'].'</b><meta http-equiv=refresh content="2;url=index.php?n=account&sub=manage">');
            }
        }
		else
		{
            output_message('validation','<b>'.$lang['bad_mail'].'</b><meta http-equiv=refresh content="2;url=index.php?n=account&sub=manage">');
        }
    }
	elseif($_GET['action']=='changepass')
	{
        ...
        if(strlen($newpass)>3)
		{
            ...
            output_message('success','<b>'.$lang['change_pass_succ'].'</b><meta http-equiv=refresh content="2;url=index.php?n=account&sub=manage">');           
        }
		else
		{
	...
	elseif($_GET['action']=='change')
	{
		...
		{
            if(@unlink('images/avatars/'.$_POST['avatarfile']))
			{
	...
	elseif($_GET['action']=='changesecretq')
	{
        ...
        isset($_POST[secreta2]) && strlen($_POST[secreta1])>4 && strlen($_POST[secreta2])>4 && $_POST['secreta1'] != $_POST['secreta2'] && $_POST['secretq1'] != $_POST['secretq2'])
		{
            ...
            output_message('success','<b>'.$lang['changed_secretq'].'</b><meta http-equiv=refresh content="4;url=index.php?n=account&sub=manage">');
        }
		else
		{
            output_message('error','<b>'.$lang['fail_change_secretq'].'</b><meta http-equiv=refresh content="3;url=index.php?n=account&sub=manage">');
        }
    }
	elseif($_GET['action']=='resetsecretq')
	{
        if ($_POST['reset_secretq'])
		{
          ...
          output_message('success','<b>'.$lang['reset_succ_secretq'].'</b><meta http-equiv=refresh content="4;url=index.php?n=account&sub=manage">');
        }
    }
	elseif($_GET['action'] == 'change_gameplay')
	{
		if($_POST['switch_wow_type']=='wotlk')
		{
			...
			output_message('success','<b>'.$lang['exp_set'].'</b><meta http-equiv=refresh content="4;url=index.php?n=account&sub=manage">');
	    }
