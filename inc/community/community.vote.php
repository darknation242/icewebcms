...
function initialize_user()
{
	...
	// Table voting_points
	$get_voting_points = $DB->select("SELECT `points`, `date`, `date_points`, `points_earned`, `points_spent` FROM `voting_points` WHERE `id` = ".$_SESSION["user_id"]." LIMIT 1");
	if (count($get_voting_points) > 0){
    foreach ($get_voting_points as $row) 
	{
		$_SESSION["points"] = ($row["points_earned"] - $row["points_spent"]);
		$today = date("Ymd");
		if($row["date"] <> $today)
		{
